...
public class Parser {
    ...
    private language_elements.type_system.Type parseType() {
        ...
        while (stream.peek().isOfType(Type.T_L_SQ_PAREN)) {
            require(Type.T_L_SQ_PAREN);
            System.out.println(t);
            t = language_elements.type_system.Type.arrayType(t);
            require(Type.T_R_SQ_PAREN);
        }
        return t;
    }
    ...
    private Statement parseStatement() {
        switch (stream.peek().getType()) {
            ...
            case T_IDENTIFIER:
                return parseAssignOrPostExpressionOrLiteral();
            default:
    ...
    private Statement parseAssignOrPostExpressionOrLiteral() {
        stream.next();
    ...
    private Statement parseVarDeclaration() {
        ...
        //Array delcaration
        if (stream.peek().isOfType(Type.T_L_SQ_PAREN)) {
            List<Expression> dim = new ArrayList<>();
            while (stream.peek().isOfType(Type.T_L_SQ_PAREN)) {
                require(Type.T_L_SQ_PAREN);

                type = language_elements.type_system.Type.arrayType(type);
                dim.add(parseExpression());

                require(Type.T_R_SQ_PAREN);
            }

            return new ArrayDeclaration(typeToken.getPosition(), type, dim, identifier);
        } else {
    ...
    private Expression parsePostExpression() {
        ...
        while ((operation = stream.peek()).isOfType(Type.T_INCREMENT) || operation.isOfType(Type.T_DECREMENT)) {
            ...
            switch (operation.getType()) {
                case T_L_SQ_PAREN:
                    left = new ArrayAccess(left, parseExpression());
                    require(Type.T_R_SQ_PAREN);
                case T_INCREMENT:
    ...
    private Expression parseLiteral() {
        ...
        switch (current.getType()) {
            ...
            case T_STRING_LITERAL:
                ...
                return new StringLiteral(current.getPosition(), CharUtility.unescapeJavaString(s.substring(1, s.length() - 1)));
